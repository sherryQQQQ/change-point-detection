...
# **Correction made:** Arrivals generated exactly as in the CoxM1 case.
...
arrival_rate_function = lambda t: arrival_rates[0] if t <= time_new_arr_rate else arrival_rates[1]
...
# Define arrival generation function
def gen_next_arrival_time(clock, # Current time
                          T # Simulation time horizon
                          ):
    
    if clock < T:
        # Compute upper bound on arrival rate based on the arrival rate function
        step_size = 0.02
        time_steps = np.arange(0, T, step_size)
        arrival_rate_array = np.array([arrival_rate_function(t) for t in time_steps])
        UB = 1.1*max(arrival_rate_array) # Upper bound for the arrival rate
        u = UB # Uniform random variable to generate next arrival time
        arr_time = clock # Next arrival time to be generated

        while arr_time < T and arrival_rate_function(arr_time) <= u:
...
filename = os.path.join('./Data',"step2M1_arr1{}_arr2{}_tchange{}_serv{}_t{}.pickle".format(arrival_rates[0],arrival_rates[1],int(10*time_new_arr_rate), service_rate, t0))
with open(filename, 'wb') as f:
   pickle.dump(X0, f)

filename = os.path.join('./Data',"step2M1_arr1{}_arr2{}_tchange{}_serv{}_t{}.pickle".format(arrival_rates[0],arrival_rates[1],int(10*time_new_arr_rate), service_rate, t1))
with open(filename, 'wb') as f:
   pickle.dump(X1, f)
...
hist_name = os.path.join('./Figures', "for_histogram_step2M1_arr1{}_arr2{}_tchange{}_serv{}_t{}.pickle".format(arrival_rates[0],arrival_rates[1],int(10*time_new_arr_rate), service_rate, t0))
...
counts, bins, bars = plt.hist(X_plot, density=True, bins=int(max(X_plot)))
dictionary = {'counts': counts, 'bins': bins, 'bars': bars}
hist_name = os.path.join('./Figures', "for_histogram_step2M1_arr1{}_arr2{}_tchange{}_serv{}_t{}.pickle".format(arrival_rates[0],arrival_rates[1],int(10*time_new_arr_rate), service_rate, t1))
with open(hist_name, 'wb') as f:
    pickle.dump(dictionary, f)
